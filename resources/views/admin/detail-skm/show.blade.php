...
@section('title', 'Preview SKM')

@section('content')
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-semibold text-slate-800">Preview SKM</h1>
...
@php
    ...
    for ($i = 1; $i <= 9; $i++) {
        ...
        if ($nrrTertimbangU_raw[$col] !== null) $sumNrrTertimbang_raw += $nrrTertimbangU_raw[$col];
...
<div
    x-data="{
        ...
        isDirty() { return JSON.stringify(this.form) !== JSON.stringify(this.original); },
        undo() { this.form = JSON.parse(JSON.stringify(this.original)); },
        submitIfDirty(e) {
            if (!this.isDirty()) { e.preventDefault(); this.closeModal(); return false; }
            return true;
        },
        init() {
...
>
    ...
    <div class="mb-6 rounded-lg border border-slate-200 bg-slate-50 p-4">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div class="rounded-lg border border-slate-200 bg-white p-3 text-center">
                <span class="text-xs text-slate-500">Nama Ruangan</span>
                <div class="mt-1 font-semibold text-slate-800">{{ $parent->nama_ruangan }}</div>
            </div>
            <div class="rounded-lg border border-slate-200 bg-white p-3 text-center">
                <span class="text-xs text-slate-500">Bulan</span>
                <div class="mt-1 font-semibold text-slate-800">{{ $parent->bulan }}</div>
            </div>
            <div class="rounded-lg border border-slate-200 bg-white p-3 text-center">
                <span class="text-xs text-slate-500">Tahun</span>
                <div class="mt-1 font-semibold text-slate-800">{{ $parent->tahun }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="mb-6 rounded-lg border border-slate-200 bg-white p-4">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-4">
            <div>
                <span class="text-xs text-slate-500">Total Respon</span>
                <div class="font-semibold text-slate-800">{{ $count }}</div>
            </div>

            <div>
                <span class="text-xs text-slate-500">SUM NRR Tertimbang</span>
                <div class="font-semibold text-slate-800">{{ $sumNrrTertimbang !== null ? $sumNrrTertimbang : '-' }}</div>
            </div>

            <div>
                <span class="text-xs text-slate-500">IKM Unit Pelayanan (N41×25)</span>
                <div class="font-semibold text-slate-800">{{ $ikmUnitPelayanan !== null ? $ikmUnitPelayanan : '-' }}</div>
            </div>

            <div>
                <span class="text-xs text-slate-500">Kategori</span>
                <div class="font-semibold text-slate-800">{{ $kategoriIkmUnit }}</div>
            </div>
        </div>
        ...
        <div class="mt-2 text-xs text-slate-500">
            ...
            <div><b>IKM Unit Pelayanan</b> = (SUM NRR tertimbang) × 25.</div>
        </div>
    </div>
    ...
    <div class="overflow-x-auto rounded-lg border border-slate-200">
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-600">No. Urut</th>
                    @for ($i=1; $i<=9; $i++)
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-600">U{{ $i }}</th>
                    @endfor
            ...
            <tbody class="divide-y divide-slate-200 bg-white">
                @forelse($details as $d)
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 text-sm font-semibold text-slate-800">{{ $loop->iteration }}</td>

                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u1 }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u2 }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u3 }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u4 }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u5 }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u6 }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u7 }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u8 }}</td>
                        <td class="px-4 py-3 text-sm text-slate-800">{{ $d->u9 }}</td>
                            ...
                            <div class="flex items-center gap-2">
                                @if($canUndoBackend)
                                    <form method="POST"
                                        ...
                                        <button type="submit"
                                                class="rounded-lg border px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                                            Undo
                                        </button>
                                    </form>
                                @endif
                                ...
                                <button type="button"
                                        class="rounded-lg border px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50"
            ...
            @if($count > 0)
                <tfoot class="bg-slate-50 border-t border-slate-200">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-700">NILAI/UNSUR</th>
                        @for($i=1; $i<=9; $i++)
                            @php $col = 'u'.$i; @endphp
                    ...
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-700">NRR/UNSUR</th>
                        @for($i=1; $i<=9; $i++)
                            @php $col = 'u'.$i; @endphp
                            <td class="px-4 py-3 text-sm font-semibold text-slate-900">
                    ...
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-700">NRR tertimbang per unsur</th>
                        @for($i=1; $i<=9; $i++)
                            @php $col = 'u'.$i; @endphp
                            <td class="px-4 py-3 text-sm font-semibold text-slate-900">
                    ...
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-700">IKM Unit Pelayanan</th>
                        @for($i=1; $i<=9; $i++)
                            <td class="px-4 py-3 text-sm text-slate-400">-</td>
                        @endfor
                        <td class="px-4 py-3 text-sm font-extrabold text-slate-900">
                    ...
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-700">NILAI (NRR×25)</th>
                        @for($i=1; $i<=9; $i++)
                            @php $col = 'u'.$i; @endphp
                            <td class="px-4 py-3 text-sm font-semibold text-slate-900">
    ...
    <div x-show="openEdit" x-cloak
        ...
        <div class="w-full max-w-2xl rounded-xl bg-white shadow-lg">
            <div class="flex items-center justify-between border-b px-5 py-4">
                <div class="font-semibold text-slate-800">Edit Detail</div>
                <button type="button"
            ...
            <form method="POST" :action="editAction" class="p-5" @submit="submitIfDirty($event)">
                ...
                <div class="mb-4 flex items-center justify-between gap-2">
                    ...
                    <button type="button"
                            x-show="isDirty()"
                            x-cloak
                            class="inline-flex items-center gap-2 rounded-lg border px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50"
                            @click="undo()">
                        Undo
                    </button>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    @for($i=1; $i<=9; $i++)
                        <div>
                            <label class="mb-1 block text-xs font-semibold text-slate-600">U{{ $i }}</label>
                            <select class="w-full rounded-lg border-slate-300 text-sm"
